...
func cmdSessionsList(args []string, out, errw io.Writer) int {
	values, bools, rest, err := splitFlags(args, map[string]bool{"source": true, "limit": true}, map[string]bool{"json": true})
	if err != nil {
		return fatalf(errw, "sessions list: %s", err)
	}
	if len(rest) != 0 {
		return fatalf(errw, "usage: miseledger sessions list [--source KIND] [--limit N] [--json]")
	}
	...
	defer db.Close()
	rows, err := listSessions(db, values["source"], limit)
	if err != nil {
		return fatalf(errw, "sessions list: %s", err)
	}
...
func cmdSessionsSearch(args []string, out, errw io.Writer) int {
	...
	if len(rest) == 0 {
		return fatalf(errw, "usage: miseledger sessions search <query> [--source KIND] [--limit N] [--json]")
	}
	...
	defer db.Close()
	rows, err := searchSessions(db, strings.Join(rest, " "), values["source"], limit)
	if err != nil {
		return fatalf(errw, "sessions search: %s", err)
	}
	if bools["json"] {
...
func listSessions(db *sql.DB, sourceKind string, limit int) ([]SessionResult, error) {
	...
	sqlText := `select s.kind, c.external_id, c.name, c.kind, count(i.id), coalesce(min(i.created_at),'') as first_seen, coalesce(max(i.created_at),'') as last_seen,
...
group by c.id, s.kind, c.external_id, c.name, c.kind
...
func searchSessions(db *sql.DB, query, sourceKind string, limit int) ([]SessionResult, error) {
	...
	sqlText := `select i.id, s.kind, c.id, c.external_id, c.name, c.kind, coalesce(i.created_at,''), coalesce(i.raw_path,''), coalesce(i.raw_ordinal,0), snippet(item_fts, 5, '[', ']', '...', 20), bm25(item_fts)
from item_fts
...
limit ?`
	...
	defer rows.Close()
	type hit struct {
		itemID, sourceKind, collectionID, externalID, name, kind, createdAt, rawPath, snippet string
		rawOrdinal                                                                            int64
	}
	...
	for rows.Next() {
		var h hit
		var score float64
		if err := rows.Scan(&h.itemID, &h.sourceKind, &h.collectionID, &h.externalID, &h.name, &h.kind, &h.createdAt, &h.rawPath, &h.rawOrdinal, &h.snippet, &score); err != nil {
			return nil, err
		}
