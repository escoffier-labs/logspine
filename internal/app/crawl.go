...
func cmdCrawl(args []string, out, errw io.Writer) int {
	if len(args) == 0 {
		return fatalf(errw, "usage: miseledger crawl sessions|docs|files|repo|markdown|html|gitlog|json|jsonl|adapter|cursor|discord|chatgpt-export|claude-export <path> [options]")
	}
	switch args[0] {
	case "sessions":
	...
	case "discord":
		return cmdCrawlDiscord(args[1:], out, errw)
