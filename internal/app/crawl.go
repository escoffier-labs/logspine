...
func cmdCrawl(args []string, out, errw io.Writer) int {
	if len(args) == 0 {
		return fatalf(errw, "usage: miseledger crawl sessions|docs|files|repo|markdown|html|gitlog|json|jsonl|adapter|cursor|discord|slack|chatgpt-export|claude-export <path> [options]")
	}
	switch args[0] {
	case "sessions":
